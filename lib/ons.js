...
module.exports = class ONS {
  constructor(app) {
    ...
    this.config = app.config.onsHttp;
    app.ready(() => {
      this.appReady = true;
    });
  ...
  async createConsumer(options, Subscribers) {
    ...
    assert(!consumerMap.has(key), `[egg-ons-http] duplicate consumer, consumerGroup=${consumer.consumerGroup}, clientId=${consumer.clientId}`);
    consumerMap.set(key, consumer);

    // await consumer.ready();
    // logger.info('[egg-ons-http] consumer: %s is ready, messageModel: %s', consumer.consumerGroup, consumer.messageModel);

    // app.beforeClose(async function() {
    //   await consumer.close();
    //   logger.info('[egg-ons-http] consumer: %s is closed, messageModel: %s', consumer.consumerGroup, consumer.messageModel);
    // });
    ...
    for (const topic of topics) {
      ...
      if (!Subscriber) {
        app.coreLogger.warn('[egg-ons-http] CANNOT find the subscription logic for topic=%s', topic);
        continue;
      }
  ...
  async createProducer(options) {
    ...
    assert(!producerMap.has(producer.producerGroup), `[egg-ons-http] duplicate producer, producerGroup=${producer.producerGroup}`);
    ...
    // logger.info('[egg-ons-http] producer: %s is ready', producer.producerGroup);

    // app.beforeClose(async function() {
    //   await producer.close();
    //   logger.info('[egg-ons-http] producer: %s is closed', producer.producerGroup);
    // });
  }

  async send(msg) {
    assert(msg && msg.topic, '[egg-ons-http] send(msg) msg.topic is required');
    const producer = this.topic2Producer.get(msg.topic);
    assert(producer, `[egg-ons-http] CANNOT find producer for topic=${msg.topic}`);
